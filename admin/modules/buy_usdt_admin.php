...
$conn->query("UPDATE admin_notifications SET is_read = 1 WHERE type = 'buy_usdt' AND is_read = 0");

$search = $conn->real_escape_string($_GET['search'] ?? '');
$from   = $conn->real_escape_string($_GET['from'] ?? '');
$to     = $conn->real_escape_string($_GET['to'] ?? '');

$where = "WHERE t.type = 'buy'";
...
$stats  = $conn->query("SELECT COUNT(*) as total_orders, SUM(amount) as total_usdt FROM user_transactions WHERE type='buy' AND status='completed'")->fetch_assoc();
$totalInr = $conn->query("SELECT SUM(CAST(REGEXP_REPLACE(SUBSTRING_INDEX(description,'using ',-1), '[^0-9.]', '') AS DECIMAL(15,2))) as s FROM user_transactions WHERE type='buy' AND status='completed'")->fetch_assoc()['s'] ?? 0;
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>Buy USDT - Admin</title>
...
<body>
<div class="wrap">
  <h2 class="mb-4">&#x1F6D2; Buy USDT Transactions</h2>

  <div class="row mb-4">
    <div class="col-md-4">
      <div class="stat-card" style="background:#198754;">
        <div style="font-size:0.85rem;opacity:0.85;">Total Buy Orders</div>
    ...
    <div class="col-md-4">
      <div class="stat-card" style="background:#0d6efd;">
        <div style="font-size:0.85rem;opacity:0.85;">Total USDT Bought</div>
    ...
    <div class="col-md-4">
      <div class="stat-card" style="background:#fd7e14;">
        <div style="font-size:0.85rem;opacity:0.85;">Total INR Spent</div>
        <div style="font-size:1.8rem;font-weight:700;">&#x20B9;<?= number_format($totalInr, 2) ?></div>
      </div>
    </div>
  </div>
  ...
  <form method="GET" class="row g-2 mb-4">
    ...
    <div class="col-md-2">
      <a href="buy_usdt_admin.php" class="btn btn-outline-secondary btn-sm w-100">Reset</a>
    </div>
  </form>
  ...
  <div class="table-responsive">
    <table class="table table-bordered table-striped table-hover align-middle">
      <thead class="table-dark">
        <tr>
          <th>#</th><th>User</th><th>USDT Bought</th><th>INR Spent</th><th>Rate / Label</th><th>Status</th><th>Date</th>
        </tr>
      </thead>
      <tbody>
        <?php
        ...
        if ($result && $result->num_rows > 0):
          while ($row = $result->fetch_assoc()):
            ...
            // Format: "Bought X USDT @ ?RATE (LABEL) using ?AMOUNT"
            preg_match('/using .([0-9,\.]+)/', $desc, $inrM);
            preg_match('/@ .([0-9\.]+)(?:\s*\(([^)]+)\))?/', $desc, $rateM);
            $inrSpent = isset($inrM[1])  ? '&#x20B9;' . str_replace(',', '', $inrM[1]) : '&mdash;';
